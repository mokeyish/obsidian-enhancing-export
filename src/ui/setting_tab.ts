...
export class ExportSettingTab extends PluginSettingTab {
  ...
  display(): void {
    ...
    let current = new Proxy(
      globalSetting.items.find(v => v.name === globalSetting.lastEditName) ??
        globalSetting.items.first(),
      settingWatcher
    );

    const changeEditExportSetting = (name: string) => {
      const newSetting =
        globalSetting.items.find(v => v.name === name) ??
        globalSetting.items.first();
    ...
    new Setting(containerEl).setName(lang.pandocPath).addText(cb => {
      cb.setPlaceholder(lang.pandocPathPlaceholder).onChange(v => {
        if (globalSetting.pandocPath !== v) {
          globalSetting.pandocPath = setPlatformValue(
            globalSetting.pandocPath,
            v
          );
        }
      });
    ...
    new Setting(containerEl)
      ...
      .addDropdown(cb => {
        ...
        globalSettingWatcher.watchOnChanged(
      ...
      .addExtraButton(cb => {
        cb.setIcon('folder').onClick(async () => {
          const retval = await ct.remote.dialog.showOpenDialog({
            defaultPath:
              getPlatformValue(globalSetting.customDefaultExportDirectory) ??
              ct.remote.app.getPath('documents'),
            properties: ['createDirectory', 'openDirectory'],
          });

          if (!retval.canceled && retval.filePaths.length > 0) {
            globalSetting.customDefaultExportDirectory = setPlatformValue(
              globalSetting.customDefaultExportDirectory,
              retval.filePaths[0]
            );
          }
        });
        globalSettingWatcher.watchOnChanged(
          ...
          value => {
            const text =
              customDefaultExportDirectorySetting.components.first() as TextComponent;
            const val = getPlatformValue(value);
            text.setValue(val ?? '');
    ...
    globalSettingWatcher.watchOnChanged('defaultExportDirectoryMode', value => {
      setVisible(
        customDefaultExportDirectorySetting.settingEl,
        value === 'Custom'
      );
    });
    ...
    new Setting(containerEl)
      ...
      .addDropdown(cb => {
        ...
        globalSettingWatcher.watchOnChanged(
          ...
          (value: ExportSetting[]) => {
            ...
            cb.setValue(
              globalSetting.lastEditName ?? globalSetting.items.first()?.name
            );
          }
        );
      ...
      .addExtraButton(button => {
        ...
        button.onClick(() => {
          globalSetting.items = globalSetting.items.filter(
    ...
    const commandAfterExportSetting = new Setting(containerEl).addText(cb => {
      ...
      pandocSettingWatcher.watchOnChanged('runCommand', (value, _, target) => {
        setVisible(
          commandAfterExportSetting.settingEl,
          target.type === 'pandoc' && value
        );
